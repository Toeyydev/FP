                    ...
                    <>
                      ...
                      {r.noShows && r.noShows.length > 0 && (
                        <div style={{ marginTop: 4, display: "flex", flexWrap: "wrap", gap: 4 }}>
                          {r.noShows.map((n, i) => { const nsp = n.noShowPax ?? n.pax; const partial = n.pax > 0 && nsp < n.pax; const review = n.countsInReports === "needs-review"; return (
                            <span key={i} title={review ? "No-show retained in reports. The booking source says Cancelled — needs review" : partial ? "Partial no-show" : "Reported no-show"} style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 600, color: "var(--danger)", background: "var(--danger-bg)", border: `1px ${review ? "dashed" : "solid"} var(--danger-line)`, borderRadius: 7, padding: "2px 7px" }}>
                              ✗ {n.name}{n.ref ? ` · ${n.ref}` : ""}{partial ? ` · ${nsp} of ${n.pax} no-show` : (n.pax ? ` · ${n.pax} pax` : "")}{review ? " · No-show + Cancelled · needs review" : ""}
                            </span>
                          ); })}
                        </div>
                      )}</td>
