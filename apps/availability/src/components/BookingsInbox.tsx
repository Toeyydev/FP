                ...
                <tbody>
                  {needMap.map((b) => (
                    <tr key={b.id}>
                      ...
                      <td>{bookingRef(b.externalRef, b.confirmationCode) || b.customerName || "—"}</td>
                      ...
                      <td style={{ textAlign: "right", whiteSpace: "nowrap" }}>
                        ...
                        <button className="btn sm danger" onClick={() => removeBooking(b.id, `${b.source} · ${bookingRef(b.externalRef, b.confirmationCode) || b.customerName || "—"}`)}>🗑 Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
                              ...
                              <div style={{ marginTop: 5, display: "flex", flexWrap: "wrap", gap: 5 }}>
                                {items.map((b) => (
                                  <span key={b.id} style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "#fff", border: "1px solid var(--line,#ddd)", borderRadius: 20, padding: "2px 6px 2px 10px", fontSize: 12 }}>
                                    <button onClick={() => openDetail(b.id)} title="Details" style={{ border: "none", background: "none", cursor: "pointer", padding: 0, font: "inherit" }}>{bookingRef(b.externalRef, b.confirmationCode) || b.customerName || "—"} ×{b.pax ?? "?"} ℹ️</button>
                                    <button title="Delete" onClick={() => removeBooking(b.id, bookingRef(b.externalRef, b.confirmationCode) || b.customerName || "—")} style={{ border: "none", background: "#fbe6e2", color: "#b23b2e", borderRadius: "50%", width: 18, height: 18, cursor: "pointer", lineHeight: 1, fontWeight: 700 }}>×</button>
                                  </span>
                                ))}
                              </div>
                            </div>
      ...
      {detail && (
        <div className="scrim show" onClick={(e) => { if (e.target === e.currentTarget) setDetail(null); }}>
          <div className="modal" style={{ maxWidth: 560, width: "92%" }}>
            <h3>Booking · {str(bookingRef(detail.externalRef as string, detail.confirmationCode as string)) || "—"}</h3>
            <div className="mbody">
