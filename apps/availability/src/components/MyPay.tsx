...
type Data = { months: Month[]; yearTotal: number; paidThisMonth: number };
...
export default function MyPay() {
  ...
  useEffect(() => { fetch("/api/my-pay", { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)).then(setD).catch(() => {}); }, []);
              ...
              <div>
                {allTours.map((t, i) => (
                  <div key={i} className="pay-row">
                    ...
                    <div className="r2"><Status paid={t.paid} paidAt={t.paidAt} /><Slip url={t.slip} /></div>
                  </div>
                ))}
              </div>
            ) : (
                  ...
                  return (
                    <div key={m.period} className="pay-mrow">
                      ...
                      {isOpen && (
                        <div className="msub">
                          {m.tours.map((t, i) => (
                            <div key={i} className="sub-r"><span className="sw">{dLabel(t.date)} · {t.tour}</span><span className="sa">{thb(t.amount)}{!t.paid ? " ⏳" : ""}</span></div>
                          ))}
                        </div>
                      )}
                      <div className="mfoot">
