...
import { detachMisattributedSlip, SlipCorrectionRefused, type CorrectionInput, type DriveOps } from "@/lib/payment-slip-correction";
import { POST } from "@/app/api/admin/payment-corrections/detach-slip/route";
...
function fakeDrive(over: Partial<Record<string, { name: string; md5Checksum: string | null; trashed?: boolean }>> = {}) {
  ...
  const calls = { renamed: [] as { id: string; name: string; description: string }[], deleted: 0 };
  const drive: DriveOps = {
    ...
    async rename(id, name, description) { calls.renamed.push({ id, name, description }); files[id] = { ...files[id], name }; return { id, name, md5Checksum: files[id].md5Checksum, trashed: false }; },
  };
  return { drive, calls };
}
...
const input = (over: Partial<CorrectionInput> = {}): CorrectionInput => ({
  ...
  rightfulGuideId: "G-902", reason: "สลิปของ G-902 ถูกแนบผิดกับงานของ G-901 (ทดสอบ)", renameDriveFile: true, ...over,
});
const actor = { id: "u_actor", name: "Malee Testsuite", role: "ADMIN" };
...
describe("the correction", () => {
  it("puts the row back to PENDING without the slip, withdraws only that unread notice, audits it, renames in place", async () => {
    ...
    expect(log).toMatchObject({ action: "pay.slip_detached", actorId: "u_actor", actorRole: "ADMIN", entityType: "TourPayment", entityId: wrongRow.id });
    const d = log.detail as Record<string, any>;
    expect(d.reason).toMatch(/แนบผิด/);
    expect(d.previous).toEqual({ status: "PAID", paidAt: "2026-08-20T07:07:11.000Z", approvedBy: admin.id, eslipUrl: link(WRONG_FILE), driveFileId: WRONG_FILE });
    ...
    expect(d.driveFileKept).toBe(true);
    ...
    expect(r.drive).toMatchObject({ renamed: true, fileId: WRONG_FILE, error: null });
  });

  it("without the rename flag, Drive is only read", async () => {
    const { drive, calls } = fakeDrive();
    await detachMisattributedSlip(input({ renameDriveFile: false }), actor, { drive });
    expect(calls.renamed).toHaveLength(0);
    expect(await prisma.auditLog.count({ where: { action: { startsWith: "drive." } } })).toBe(0);
  });

  it("a failed rename leaves the correction in place and says so in the audit", async () => {
    const { drive } = fakeDrive();
    drive.rename = async () => { throw new Error("Drive rename failed: HTTP 403"); };
    const r = await detachMisattributedSlip(input(), actor, { drive });
    expect(r.drive).toMatchObject({ renamed: false, error: "Drive rename failed: HTTP 403" });
...
describe("the route: ADMIN only, and the body never names the actor", () => {
  ...
  it("GUIDE, OPERATOR and ACCOUNTANT get 403, nobody signed in gets 401, a revoked ADMIN gets 403 — and nothing changes", async () => {
    ...
    for (const role of ["GUIDE", "OPERATOR", "ACCOUNTANT"]) {
      ...
      as(u.id, role);
      expect((await call(input())).status, role).toBe(403);
    }
    as(null, null);
  ...
  it("a body that names the actor or the time is refused", async () => {
    ...
    for (const forged of [{ actorId: "u_x" }, { actorName: "Someone" }, { correctedAt: "2026-01-01T00:00:00Z" }]) {
      expect((await call({ ...input(), ...forged })).status).toBe(400);
    }
