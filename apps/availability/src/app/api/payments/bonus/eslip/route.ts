...
// POST (multipart) { bonusId, file } — upload a bonus payment slip, store it, link it.
export async function POST(req: NextRequest) {
  ...
  const { link } = await saveEslip({ base64, mimeType: mime, filename: name, replaceUrl: bonus.eslipUrl });
