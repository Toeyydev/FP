...
function page(title: string, body: string, tone: "ok" | "info" | "bad" = "info") {
  ...
  const icon = tone === "ok" ? "✅" : tone === "bad" ? "⚠️" : "ℹ️";
  const html = `<!doctype html><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><div style="font-family:system-ui,-apple-system,sans-serif;max-width:440px;margin:16vh auto;padding:0 24px;text-align:center;color:#2a2520"><div style="font-size:44px">${icon}</div><h1 style="font-size:22px;margin:14px 0 8px;color:${color}">${title}</h1><p style="color:#6f665b;font-size:15px;line-height:1.55">${body}</p><p style="margin-top:22px"><a href="https://guide.folkpaths.com/" style="display:inline-block;background:#7e3a2c;color:#fff;text-decoration:none;font-weight:700;padding:12px 22px;border-radius:12px">Open Folkpaths</a></p></div>`;
...
export async function GET(req: NextRequest) {
  ...
  const msg = res.reason === "taken" ? "Another guide already accepted this job." : res.reason === "expired" ? "This offer has expired and was handed back to the operator." : "This offer is no longer open.";
  return page("Couldn’t accept", msg, "bad");
}
