...
import { rejectReceipt } from "@/lib/advances/service";

export const dynamic = "force-dynamic";

// POST { reason } — the money never arrived. The row stays as the record of the claim.
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const result = await rejectReceipt(prisma, {
    receiptId: id, reason: body.reason ?? "",
