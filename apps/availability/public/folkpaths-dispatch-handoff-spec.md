...
## 13. Security (summary — full list in `folkpaths-security-checklist.md`)

Risk model: guide phone numbers, customer booking PII, and Google OAuth tokens sit behind a single operator account. Access control and account security dominate the risk.

Must-have before launch:
- **Server-side authorization on every endpoint** — guides reach only their own data; operator routes reject non-operators. Never trust client-sent role/IDs. (Highest priority.)
- **Strong auth** — argon2/bcrypt password hashing, login rate-limiting/lockout, **2FA on the operator account** and the Google account.
- **Sessions** — HttpOnly/Secure/SameSite cookies, rotate on login, real server-side logout.
- **HTTPS + HSTS + security headers** (CSP, X-Frame-Options, nosniff, Referrer-Policy).
- **Secrets in env vars, not the repo** — `.env` gitignored; rotate anything ever committed.
- **Encrypt Google refresh tokens at rest**; `calendar.events` scope only; revocable.
- **Input validation + parameterized queries**; escape output (XSS).

Soon after: privacy notice + consent and data-retention/deletion for **Thailand PDPA** (consult locally — not legal advice); encrypted daily backups; dependency scanning; auth-event logging without PII; PWA cache hygiene (no PII cached past logout).

Note: platform choice (PWA vs native) does not change these — all are server-side.

---

## 14. Guide trust & retention features

These exist for one reason: anxious guides stop updating their availability, and stale availability breaks dispatch. Keeping the guide experience calm and fair is what keeps calendars current.

### 14.1 On-tour pause ("hold my offers")
A guide presence sub-state: `ON_TOUR`. While set, the guide receives **no** new offers; they queue silently in the Jobs tab.
- **Auto-on** during a confirmed tour's time window (derived from the job sheet); **auto-off** when it ends. Guide can also toggle manually anytime.
- **Fairness rule:** missing an offer while `ON_TOUR` must NOT count against the guide in any ranking/priority metric. Make this explicit in code and tell the guide ("won't count against you").
- Operator board shows `ON_TOUR` distinctly from plain offline (e.g. "On a tour until 11:30") so the operator knows not to wait on them for a first-to-accept.
- Interaction with offers: when an offer is broadcast, skip `ON_TOUR` guides for the instant push but still place it in their Jobs tab; they can accept on return if still open.

### 14.2 Forward availability nudge
A home-screen card giving the guide forward visibility (cures the "is next week empty?" anxiety and the passivity of only-reactive offers).
- Shows next 7 days: tours booked + days with **no availability set** flagged (dashed cells).
- Copy: "You have {n} tours booked. {days} are still empty — add your availability so you don't miss work."
- One-tap "Add {days}" jumps to those days in the calendar.
- Pair with the existing weekly availability reminder (§11). Tone: encouraging, never guilt-tripping.

### 14.3 Payment transparency on the job sheet
Add a payment block to the JobSheet (§7) so the guide always knows the money, which builds trust per tour.
- Fields: `amount`, `method` (e.g. cash-from-guests / paid-by-operator), `terms` (e.g. "due on tour day" / "settled weekly, paid Fridays"), `tipsPolicy`, `status` (`DUE` / `PAID`).
- Status updates when the operator marks paid; guide sees `PAID` confirmation.
- These are operator-configurable per product (set once, not per job).

### 14.4 Offer fairness (cross-cutting)
- No penalty for: offers missed while `ON_TOUR`, offline, or on a day off.
- If you implement any guide ranking ("best match"), base it on skills/language/rating/proximity — never on raw response speed alone, or guides feel they must watch the phone constantly. Document the ranking inputs so it's explainable to guides.
